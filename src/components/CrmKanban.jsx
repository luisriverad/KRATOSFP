    ...
    <div>
      <div className="flex items-end justify-between mb-4 gap-4">
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <button onClick={() => setShowNew(true)} className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg bg-kratos-ink text-white text-sm font-medium hover:opacity-90 transition">
        ...
        {ETAPAS.map(col => {
          ...
          return (
            <div key={col.key}
              ...
              className={`w-72 shrink-0 rounded-xl border transition-colors ${overCol === col.key ? 'border-kratos-ink bg-kratos-panel-2' : 'border-kratos-border bg-kratos-bg2/40'}`}>
              ...
              <div className="px-3 py-3 border-b border-kratos-border" style={{ borderTop: `3px solid ${col.color}`, borderTopLeftRadius: 12, borderTopRightRadius: 12 }}>
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] font-mono text-kratos-muted bg-kratos-panel px-1.5 py-0.5 rounded">{items.length}</span>
                </div>
              ...
              <div className="p-2 space-y-2 min-h-[120px]">
                {items.map(d => (
                  <div key={d.id}
                    ...
                    className={`group bg-kratos-panel border border-kratos-border rounded-lg p-3 cursor-pointer hover:border-kratos-ink/40 hover:shadow-soft transition ${dragId === d.id ? 'opacity-40' : ''}`}>
      ...
      {sel && createPortal(
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" onClick={() => setSelId(null)}>
          ...
          <div className="relative panel w-full max-w-4xl p-6 shadow-2xl max-h-[88vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            {/* Recomendación IA */}
            <div className="surface-2 rounded-lg p-3 mb-5 border-l-4 border-l-kratos-info">
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" onClick={() => setShowNew(false)}>
          ...
          <form onSubmit={crear} className="relative panel w-full max-w-xl p-6 shadow-2xl max-h-[88vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center gap-2 mb-4">
              <span className="w-9 h-9 rounded-lg bg-kratos-ink text-white flex items-center justify-center"><Plus size={16} /></span>
              <div>
            ...
            <button type="submit" disabled={!form.empresa.trim()}
              className="mt-5 px-4 py-2.5 bg-kratos-ink text-white rounded-lg text-sm font-medium hover:opacity-90 transition disabled:opacity-40">
              Crear oportunidad
            </button>
          </form>
        </div>,
